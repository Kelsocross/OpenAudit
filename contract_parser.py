...
class ContractParser:
    ...
    def _parse_pdf(self, file_content: bytes) -> ContractTerms:
        """Parse PDF contract files using PyMuPDF"""
        terms = ContractTerms()
        extracted_text = ""
        ...
        try:
            ...
            for page in doc:
                ...
                if hasattr(page, 'find_tables'):
                    page_tables = page.find_tables()  # type: ignore
                    if page_tables:
    ...
    def _extract_zone_rates_from_tables(self, tables: List) -> Optional[Dict[str, float]]:
        ...
        for table in tables:
            ...
            try:
                ...
                df = pd.DataFrame(table[1:], columns=columns)  # type: ignore
                ...
                if zone_cols and rate_cols:
                    for _, row in df.iterrows():
                        zone_val = row.get(zone_cols[0])
                        rate_val = row.get(rate_cols[0])
                        zone = str(zone_val) if zone_val is not None else None
                        
                        if zone and rate_val is not None:
                            try:
                                # Clean up rate value
                                rate_clean = re.sub(r'[^\d\.]', '', str(rate_val))
                                if rate_clean:
        ...
        try:
            ...
            if zone_cols and rate_cols:
                for _, row in df.iterrows():
                    ...
                    if not pd.isna(rate_val):  # type: ignore
                        try:
                            # Clean up rate value
